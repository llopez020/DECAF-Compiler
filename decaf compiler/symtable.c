...
/*   Levi Lopez
     ...
     Changelog: 
     - Added global variable to keep track of temporary symbol names 
     - Added function to create a temporary symbol name 
     - Added more Types and Subtypes to PrintSym
     - Slightly changed formatting of Display
*/
...
int GTEMP=0; // global number of temporary symbol names

/* char * TEMP_CREATE() */
/* PRE: nothing */
/* POST: temporary symbol name */
char * TEMP_CREATE()
{
	char s[100];
...
void PrintSym(struct SymbTab *s)
{
	char *string,*display_type; 
	 switch(s->SubType) {
		 ...
		 case ID_Sub_Type_Extern: string = "Extern " ; break;
